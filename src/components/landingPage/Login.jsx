...
import DropdownIdiomas from './DropdownIdiomas';
...
const Login = () => {
    ...
    const setStatusOnline = async (userID) => {
        try {
    ...
    return (
        <div className='flex flex-col max-w-lg max-h-screen bg-white px-16 py-8 rounded-lg'>
            ...
            <div>
                <label htmlFor={'email'} className='font-poppins font-normal text-lg text-[#555555]'>
                    Email address
                </label>
                <input
                    id={'email'}
            ...
            <div>
                <label htmlFor={'password'} className='font-poppins font-normal text-lg text-[#555555]'>
                    Password
                </label>
                <input
                    id={'password'}
            ...
            <button
                ...
                className='hover:scale-[1.01] active:scale-[0.99] active:duration-75 transition-all ease-in-out
                        font-poppins bg-[#803FE0] rounded-lg py-4 text-white mb-4 mt-10'>
                Login now
            </button>
            <div className='flex items-center justify-center mt-4 mb-4'>
                <div className='w-full border-t border-[#C2C2C2]'></div>
                <span className='font-poppins font-semibold text-[#C2C2C2] mx-4'>OR</span>
                <div className='w-full border-t border-[#C2C2C2]'></div>
            </div>
            <button
                onClick={() => setShowModal(true)}
                className='hover:scale-[1.01] active:scale-[0.99] active:duration-75 transition-all ease-in-out
                        font-poppins border-2 border-[#6200D1] text-[#6200D1] rounded-lg py-4 mt-4'>
                Forgot my password
            </button>
            {showModal && <PasswordModal onClose={handleClose} />}
        </div>
    );
};
