...
const PasswordModal = ({ onClose }) => {
    ...
    const handleForgotPassword = async () => {
        try {
            const response = await fetch('https://wf8ih91spk.execute-api.us-east-1.amazonaws.com/forgot-password/EN', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                alert('An email has been sent with the instructions to reset your password'); // Mensaje de éxito
            ...
            <div className='max-w-xl max-h-screen bg-white px-12 py-12 rounded-lg'>
                ...
                <div className='flex flex-col items-center justify-center'>
                    ...
                    <div className='flex flex-col text-center'>
                        <p className='font-poppins font-semibold text-[#555555] mt-4 mb-4'>Find your account</p>
                        <p className='font-poppins font-semibold text-[#555555] mb-4'>Please enter your email</p>
                    </div>
                    <div className='mt-4'>
                        <label htmlFor={'emailForPassword'} className='font-poppins font-normal text-lg text-[#555555]'>Email address</label>
                        <input
                            id={'emailForPassword'}
                            autoComplete={'email'}
                            type='email'
                ...
                {/* Botón de envío para la recuperación de contraseña */}
                <div className='mt-4 flex flex-col gap-y-4'>
                    <button
                        onClick={handleForgotPassword}
                        className='hover:scale-[1.01] active:scale-[0.99] active:duration-75 transition-all ease-in-out
                        font-poppins bg-[#803FE0] rounded-lg py-4 text-white mb-4'>
                        Next
                    </button>
                </div>
            </div>
