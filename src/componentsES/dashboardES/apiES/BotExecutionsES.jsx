...
const BotExecutionsES = ({ botId, onBack }) => {
    ...
    return (
        <div className="rounded-lg bg-white h-full flex flex-col">
            <div>
            <button onClick={onBack} className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-1 px-3 rounded mx-auto mb-4">Volver</button>
                <h2 className="text-lg font-semibold mb-4">Ejecuciones de Bot</h2>
                <div className="flex justify-between mb-4">
                    <button onClick={getExecutions} className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-1 px-3 rounded">Recargar Ejecuciones</button>
                    <input
                        type="text"
                        placeholder="Buscar por ID de Ejecución"
                ...
                <div className="flex justify-end mb-4">
                    <CSVLink {...exportToCSV()} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-1 px-3 rounded mr-2">Exportar CSV</CSVLink>
                    <button onClick={exportToPDF} className="bg-red-600 hover:bg-red-700 text-white font-semibold py-1 px-3 rounded mr-2">Exportar PDF</button>
                    <button onClick={exportToExcel} className="bg-green-600 hover:bg-green-700 text-white font-semibold py-1 px-3 rounded">Exportar Excel</button>
                </div>
                {isMobileView ? (
                    <div className="grid grid-cols-1 gap-4 overflow-y-auto max-h-96">
                        {filteredAndSortedExecutions.map((execution) => (
                            <div key={execution.executionID} className="bg-white shadow-xl rounded-lg p-4">
                                <div><strong>ID de Ejecución:</strong> {execution.executionID}</div>
                                <div><strong>ID de Bot:</strong> {execution.botID}</div>
                                ...
                                <div><strong>Hora de Inicio:</strong> {new Date(execution.executionStartTime).toLocaleString()}</div>
                                <div><strong>Hora de Fin:</strong> {new Date(execution.executionEndTime).toLocaleString()}</div>
                                <div><strong>Duración (s):</strong> {execution.executionDuration}</div>
                                <div><strong>Uso de CPU:</strong> {execution.cpuUsage}</div>
                                <div><strong>Uso de Memoria:</strong> {execution.memoryUsage}</div>
                                <div><strong>Pasos Completados:</strong> {execution.stepsCompleted}</div>
                                <div><strong>Respuesta:</strong> {execution.response}</div>
                            </div>
                        ))}
                    </div>
                ) : (
                    ...
                    <table className="w-full text-sm text-center text-gray-500 mb-8">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                ...
                                ID de Ejecución {sortConfig.key === 'executionID' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                                </th>
                                <th scope="col" className="px-6 py-3 cursor-pointer" onClick={() => handleSort('botID')}>
                                    ID de Bot {sortConfig.key === 'botID' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                                </th>
            ...
            <div className="mt-8">
                <h2 className="text-lg font-semibold mb-4">Estadísticas de Ejecuciones</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="bg-white shadow-md rounded-lg p-4 execution-status-chart">
                        <h3 className="font-semibold mb-2">Status de Ejecución</h3>
                    ...
                    <div className="bg-white shadow-md rounded-lg p-4 execution-duration-chart">
                        <h3 className="font-semibold mb-2">Duración de Ejecución</h3>
                        <ResponsiveContainer width="100%" height={300}>
                    ...
                    <div className="bg-white shadow-md rounded-lg p-4 cpu-usage-chart">
                        <h3 className="font-semibold mb-2">Uso de CPU</h3>
                        <ResponsiveContainer width="100%" height={300}>
                    ...
                    <div className="bg-white shadow-md rounded-lg p-4 memory-usage-chart">
                        <h3 className="font-semibold mb-2">Uso de Memoria</h3>
                        <ResponsiveContainer width="100%" height={300}>
                    ...
                    <div className="bg-white shadow-md rounded-lg p-4 steps-completed-chart">
                        <h3 className="font-semibold mb-2">Pasos Completados</h3>
                        <ResponsiveContainer width="100%" height={300}>
                    ...
                    <div className="bg-white shadow-md rounded-lg p-4 error-frequency-chart">
                        <h3 className="font-semibold mb-2">Frecuencia de Error</h3>
                        <ResponsiveContainer width="100%" height={300}>
...
export default BotExecutionsES;
